...
def worker(thread_id, num_messages):
    try:
        for i in range(num_messages):
            ...
            logger.log(level, f"Thread {thread_id:03d} - Message {i+1:03d}")
...
print(f"=== STRESS TEST: High Concurrency ===")
...
print(f"Throughput: {(num_threads * messages_per_thread) / (end_time - start_time):.0f} messages/second")
...
with open(log_file, 'r') as f:
    ...
    if len(log_lines) == expected:
        print(f"✓ All {expected} messages written successfully!")
    else:
        print(f"✗ Expected {expected} but got {len(log_lines)}")
...
if result is True:
    ...
    print("✓ Thread-safe implementation is ROCK-SOLID!")
else:
